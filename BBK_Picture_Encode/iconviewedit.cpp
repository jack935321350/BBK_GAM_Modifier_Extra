...
void IconViewEdit::LoadImageBinary(QImage image, int gaussianM, BinAlg binAlg)
{
    ...
    uint8_t *blurImg = new uint8_t[_IconW * _IconH];
    if(blurImg == nullptr) {
        delete[] grayImg;
        return;
    }
    ...
    if(gaussianM == 0) {
        memcpy_s(blurImg, _IconW * _IconH, grayImg, _IconW * _IconH);
    } else {
        if(gaussianM % 2 == 0) {
            delete[] grayImg;
            delete[] blurImg;
        ...
        gussianBlur(grayImg, blurImg, _IconW, _IconH, gaussianM);
    }
    ...
    if(binAlg == BinAlg_Otsu) {
        unsigned char thresh = calc_otsu(blurImg, _IconW, _IconH);
        threshold(blurImg, binImg, _IconW, _IconH, thresh);
    }
    else if(binAlg == BinAlg_Wellner) {
        adaptive_threshold_S(blurImg, binImg, _IconW, _IconH, 5);
    }
    else {
...
void IconViewEdit::LoadImageBinaryAll(QImage image, int gaussianM, BinAlg binAlg)
{
    QImage rsImg;
    rsImg = image.scaled(_IconW * _IconHorCnt, _IconH * _IconVerCnt).convertToFormat(QImage::Format_RGB32);
    for(int ver = 0; ver < _IconVerCnt; ver++) {
        for(int hor = 0; hor < _IconHorCnt; hor++) {
            SetIconIndex(ver * _IconHorCnt + hor);
            QImage rsImgSeg;
            rsImgSeg = rsImg.copy(hor * _IconW, ver * _IconH, _IconW, _IconH);
            LoadImageBinary(rsImgSeg, gaussianM, binAlg);
        }
    }
    memcpy_s( _IconSrcData, ICON_DATA_MAX_SIZE, _IconData, ICON_DATA_MAX_SIZE);
}
