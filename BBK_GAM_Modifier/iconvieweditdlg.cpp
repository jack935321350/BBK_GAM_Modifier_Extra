    ...
    , ui(new Ui::IconViewEditDlg)
{
    ...
    connect(ui->saveAsBtn,              &QPushButton::clicked,              this, &IconViewEditDlg::on_pushButton_clicked);
...
void IconViewEditDlg::on_pushButton_clicked()
{
    ...
    if(btn == ui->brushBtn) {
    ...
    else if(btn == ui->saveAsBtn) {

        QSettings settings("QtProject", "BBK_GAM_Modifier");
        QFile iconFile(QFileDialog::getSaveFileName(this, tr("Save File"), settings.value("last_icon_dir").toString(), "ACP(*.acp);;GDP(*.gdp)"));
        if(!iconFile.open(QFile::WriteOnly)) {
            return;
        }

        settings.setValue("last_icon_dir", QFileInfo(iconFile).absoluteDir().absolutePath());
        settings.sync();
        ...
        ui->iconViewEdit->GetIconHead(&data);
        iconStream.writeRawData((const char *)data, 6);

        uint32_t len = calc_effects_icon_size(reinterpret_cast<effects_icon_head_t *>(data)) - 6;

        ui->iconViewEdit->GetIconData(&data);
        iconStream.writeRawData((const char *)data, len);
