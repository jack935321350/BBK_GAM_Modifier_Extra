...
MainWidget::MainWidget(QWidget *parent) : QWidget(parent), ui(new Ui::MainWidget)
{
    ...
    playerBoxMenu = new QMenu(this);
    playerSaveData = new QAction(tr("Save Data"), this);
    playerBoxMenu->addAction(playerSaveData);
    connect(playerSaveData, &QAction::triggered, this, &MainWidget::on_playerSaveData);

    enemyBoxMenu = new QMenu(this);
    enemySaveData = new QAction(tr("Save Data"), this);
    enemyBoxMenu->addAction(enemySaveData);
    connect(enemySaveData, &QAction::triggered, this, &MainWidget::on_enemySaveData);

    propBoxMenu = new QMenu(this);
    propSaveData = new QAction(tr("Save Data"), this);
    propBoxMenu->addAction(propSaveData);
    connect(propSaveData, &QAction::triggered, this, &MainWidget::on_propSaveData);

    magicBoxMenu = new QMenu(this);
    magicSaveData = new QAction(tr("Save Data"), this);
    magicBoxMenu->addAction(magicSaveData);
    connect(magicSaveData, &QAction::triggered, this, &MainWidget::on_magicSaveData);
    ...
    // setWindowTitle(tr("BBK_GAM_Modifier_") + buildDateStr + '_' + buildTimeStr);
    ...
    setWindowTitle(tr("BBK_GAM_Modifier_") + buildDateTimeStr);
    ...
    connect(ui->allPlayer9999Btn,       &QPushButton::clicked,          this, &MainWidget::on_pushButton_clicked);
...
void MainWidget::on_pushButton_clicked()
{
    ...
    if(btn == ui->loadGamBtn) {
        ...
        setWindowTitle(tr("BBK_GAM_Modifier ") + buildDateTimeStr);
        ...
        QByteArray customEngine = customEngineFile.readAll();

        if(customEngineFile.size() != engineBA.size()) {
            QMessageBox::warning(this, tr("Warning"), tr("Eng Size Incorrect!"));
            return;
        }

        for(int i = 6; i < 16;i++) {
            customEngine[i] = engineBA[i];
        }
        engineBA = customEngine;
    ...
    else if(btn == ui->allPlayer9999Btn) {
        ...
        if(engineBA != newEngineBA) {
            if(QMessageBox::warning(this, tr("Warning"), tr("Please Ensure New Engine First!"), QMessageBox::Ok | QMessageBox::Cancel) != QMessageBox::Ok) {
                return;
            }
        }
        ...
        for(int i = 0; i < playerIndexVec.size(); i++) {
            ...
            player->attack_power = 9999;
            player->defense_power = 9999;
            player->body_movement = 127;
            player->spirit_power = 127;
            player->lucky_value = 127;
            player->hp_max = 999;
            player->hp = 999;
            player->mp_max = 999;
            player->mp = 999;
        ...
        QMessageBox::information(this, tr("Information"), tr("All Player 9999 OK!"));
        ...
        QSettings settings("QtProject", "BBK_GAM_Modifier");
        QFile gamFile(QFileDialog::getSaveFileName(this, tr("Save File"), settings.value("last_dir").toString() + '/' + settings.value("last_file").toString(), "GAM(*.gam)"));
...
{
    if(engineBA.isEmpty() || libraryBA.isEmpty() || playerIndexVec[index - 1] == 0) {
        return;
    }
...
{
    if(engineBA.isEmpty() || libraryBA.isEmpty() || playerIndexVec[ui->playerIdx->value() - 1] == 0) {
        return;
    }
...
{
    if(engineBA.isEmpty() || libraryBA.isEmpty() || enemyIndexVec[index - 1] == 0) {
        return;
    }
...
{
    if(engineBA.isEmpty() || libraryBA.isEmpty() || enemyIndexVec[ui->enemyIdx->value() - 1] == 0) {
        return;
    }
...
void MainWidget::on_propData_valueChanged()
{
    ...
    if(propIndexVecVec[ui->propType->currentIndex()][ui->propIdx->value() - 1] == 0) {
        return;
    }
...
void MainWidget::on_magicData_valueChanged()
{
    ...
    if(magicIndexVecVec[ui->magicType->currentIndex()][ui->magicIdx->value() - 1] == 0) {
        return;
    }
...
void MainWidget::on_playerSaveData()
{
    if(engineBA.isEmpty() || libraryBA.isEmpty() || playerIndexVec[ui->playerIdx->value() - 1] == 0) {
        return;
    }
    ...
    dataStream.writeRawData((const char *)player, sizeof(player_t));

    file.close();
}

void MainWidget::on_enemySaveData()
{
    if(engineBA.isEmpty() || libraryBA.isEmpty() || enemyIndexVec[ui->enemyIdx->value() - 1] == 0) {
        return;
    }
    ...
    QDataStream dataStream(&file);
    dataStream.writeRawData((const char *)enemy, sizeof(enemy_t));

    file.close();
}

void MainWidget::on_propSaveData()
{
    if(engineBA.isEmpty() || libraryBA.isEmpty()) {
        return;
    }
    ...
    QSettings settings("QtProject", "BBK_GAM_Modifier");
    QFile file(QFileDialog::getSaveFileName(this, tr("Save File"), settings.value("last_grs_dir").toString(), "GRS(*.grs)"));
    if(!file.open(QFile::WriteOnly)) {
        return;
    }

    settings.setValue("last_grs_dir", QFileInfo(file).absoluteDir().absolutePath());
    settings.sync();

    prop_t *prop = reinterpret_cast<prop_t *>(libraryBA.data() + propIndexVecVec[ui->propType->currentIndex()][ui->propIdx->value() - 1]);

    QDataStream dataStream(&file);
    dataStream.writeRawData((const char *)prop, sizeof(prop_t));

    file.close();
}

void MainWidget::on_magicSaveData()
{
    if(engineBA.isEmpty() || libraryBA.isEmpty()) {
        return;
    }
    ...
    QSettings settings("QtProject", "BBK_GAM_Modifier");
    QFile file(QFileDialog::getSaveFileName(this, tr("Save File"), settings.value("last_mrs_dir").toString(), "MRS(*.mrs)"));
    if(!file.open(QFile::WriteOnly)) {
        return;
    }

    settings.setValue("last_mrs_dir", QFileInfo(file).absoluteDir().absolutePath());
    settings.sync();

    magic_t *magic = reinterpret_cast<magic_t *>(libraryBA.data() + magicIndexVecVec[ui->magicType->currentIndex()][ui->magicIdx->value() - 1]);

    QDataStream dataStream(&file);
    dataStream.writeRawData((const char *)magic, sizeof(magic_t));

    file.close();
}

void MainWidget::on_playerBox_customContextMenuRequested(const QPoint &pos)
{
    Q_UNUSED(pos)
    playerBoxMenu->exec(QCursor::pos());
}

void MainWidget::on_enemyBox_customContextMenuRequested(const QPoint &pos)
{
    Q_UNUSED(pos)
    enemyBoxMenu->exec(QCursor::pos());
}

void MainWidget::on_propBox_customContextMenuRequested(const QPoint &pos)
{
    Q_UNUSED(pos)
    propBoxMenu->exec(QCursor::pos());
}

void MainWidget::on_magicBox_customContextMenuRequested(const QPoint &pos)
{
    Q_UNUSED(pos)
    magicBoxMenu->exec(QCursor::pos());
}
